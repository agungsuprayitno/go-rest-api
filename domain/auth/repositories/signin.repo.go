...
func SignIn(ctx *gin.Context) (tokenResp responses.TokenResponse, signinErr error) {
	...
	if result.Error != nil {
		forbiddenErr.SetError(ctx, "Invalid Username or Password.")
	...
	// Generate Tokens
	accessToken, err3 := utils.CreateToken(config.AccessTokenExpiresIn, user.Email, config.OauthTokenKey)
	if err3 != nil {
	...
	refreshToken, err4 := utils.CreateToken(config.RefreshTokenExpiresIn, user.Email, config.OauthRefreshTokenKey)
	if err4 != nil {
