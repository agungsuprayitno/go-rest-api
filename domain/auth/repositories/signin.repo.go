...
func SignIn(ctx *gin.Context) (tokenResp responses.TokenResponse, signinErr error) {
	...
	// Generate Tokens
	accessToken, err3 := utils.CreateToken(config.AccessTokenExpiresIn, user.ID, config.AccessTokenPrivateKey)
	if err3 != nil {
	...
	refreshToken, err4 := utils.CreateToken(config.RefreshTokenExpiresIn, user.ID, config.RefreshTokenPrivateKey)
	if err4 != nil {
