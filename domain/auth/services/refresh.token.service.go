...
func RefreshAccessToken(ctx *gin.Context) (token responses.TokenResponse, err error) {
	...
	sub, err := utils.ValidateToken(refreshToken, config.OauthRefreshTokenKey)
	if err != nil {
	...
	accessToken, err := utils.CreateToken(config.AccessTokenExpiresIn, user.Email, config.OauthTokenKey)
	if err != nil {
		forbiddenErr.SetError(ctx, err.Error())
		return
	}
